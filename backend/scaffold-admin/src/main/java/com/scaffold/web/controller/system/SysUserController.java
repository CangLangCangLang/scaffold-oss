...
public class SysUserController extends BaseController
{
    ...
    @AuditLog(module = "system.user", action = "CREATE", resourceType = "user",
            resourceId = "#user.userId", comment = "'新建用户 ' + #user.userName")
    @PostMapping
    ...
    @AuditLog(module = "system.user", action = "UPDATE", resourceType = "user",
            resourceId = "#user.userId",
            beforeProvider = "@sysUserServiceImpl.selectUserById(#user.userId)",
            comment = "'修改用户 ' + #user.userName")
    @PutMapping
    ...
    @AuditLog(module = "system.user", action = "DELETE", resourceType = "user",
            resourceId = "T(java.util.Arrays).toString(#userIds)",
            comment = "'删除用户 ID: ' + T(java.util.Arrays).toString(#userIds)",
            recordReturn = false)
    ...
    @Log(title = "用户管理", businessType = BusinessType.UPDATE)
    @AuditLog(module = "system.user", action = "RESET_PASSWORD", resourceType = "user",
            resourceId = "#user.userId",
            comment = "'重置用户 ' + #user.userName + ' 的密码'", recordReturn = false)
    @PutMapping("/resetPwd")
    ...
    @PreAuthorize("@ss.hasPermi('system:user:edit')")
    @Log(title = "用户管理", businessType = BusinessType.UPDATE)
    @AuditLog(module = "system.user", action = "CHANGE_STATUS", resourceType = "user",
            resourceId = "#user.userId",
            beforeProvider = "@sysUserServiceImpl.selectUserById(#user.userId)",
            comment = "'修改用户 ' + #user.userName + ' 状态为 ' + #user.status")
